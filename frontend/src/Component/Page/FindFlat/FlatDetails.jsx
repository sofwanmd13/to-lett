  ...
  const getFlatDetails = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/flatDetails/${id}`
      );
    ...
    <>
      ...
      <div className="w-11/12 mx-auto lg:flex border-2 border-black mt-3 rounded-lg">
        <div className="lg:w-[50%]">
          <img
            src={`http://localhost:5000/image/${flatDetails?.flatList?.images[0]}`}
            alt=""
        ...
        <div className="lg:w-[50%] grid grid-cols-1 md:grid-cols-2  h-[500px] ">
          <div className="bg-cover overflow-hidden relative border-2 border-black">
            <img
              src={`http://localhost:5000/image/${flatDetails?.flatList?.images[1]}`}
              alt=""
          ...
          <div className="bg-cover overflow-hidden relative border-2 border-black">
            <img
              src={`http://localhost:5000/image/${flatDetails?.flatList?.images[2]}`}
              alt=""
              className="w-full h-full transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-125  duration-300"
            />
          </div>
          <div className="bg-cover overflow-hidden relative border-2 border-black">
            <img
              src={`http://localhost:5000/image/${flatDetails?.flatList?.images[3]}`}
              alt=""
              className="w-full h-full transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-125  duration-300"
            />
          </div>
          <div className="bg-cover overflow-hidden relative border-2 border-black">
            <img
              src={`http://localhost:5000/image/${flatDetails?.flatList?.images[4]}`}
              alt=""
              className="w-full h-full transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-125  duration-300"
            />
                ...
                <div className="mb-16">
                  <div className="mb-5">
                    ...
                    <p className="text-black  font-medium inline-block md:text-lg mt-1">
                      Location:
                      {flatDetails?.flatList?.description?.location?.address},
                  ...
                  <div className="border-t-4 border-b-4">
                    ...
                    <ul className="mb-8 lg:text-xl  text-black">
                      ...
                      <li>
                        {" "}
                        - Bed: {
                          flatDetails?.flatList?.description?.bedroom
                        }{" "}
                        bedroom
                      </li>
                      <li>
                      ...
                      <li>
                        {" "}
                        - Size: {flatDetails?.flatList?.description?.size} size
                      </li>
                    </ul>
                  </div>
                ...
                <div>
                  <div className="flex items-center justify-between">
                    <h2 className="text-3xl font-bold my-5">
                      ${flatDetails?.flatList?.description?.rent}
                    </h2>
                    <svg
                      width={30}
                  ...
                  >
                    ...
                    +88 {flatDetails?.flatList?.contact_person?.phone}
                  </button>
                </div>
              </div>
